...
const Breadcrumbs = ({
    ...
    breadcrumbContent = (
      <Card sx={card === false ? { mb: 3, bgcolor: 'transparent', ...sx } : { mb: 3, bgcolor: 'background.default', ...sx }} {...others}>
        <Box sx={{ p: 2, pl: card === false ? 0 : 2 }}>
          ...
          >
            {/* ▼▼▼ CORREÇÃO 1 AQUI ▼▼▼ */}
            {title && !titleBottom && <BTitle title={item.breadcrumb || item.title} />}
            <Grid item>
              <MuiBreadcrumbs
  ...
  if ((item && item.type === 'item') || (item?.type === 'group' && item?.url) || custom) {
    // ▼▼▼ CORREÇÃO 2 AQUI ▼▼▼
    itemTitle = item?.breadcrumb || item?.title;
        ...
        <Card sx={card === false ? { mb: 3, bgcolor: 'transparent', ...sx } : { mb: 3, bgcolor: 'background.default', ...sx }} {...others}>
          <Box sx={{ p: 2, pl: card === false ? 0 : 2 }}>
            <Grid
              ...
              spacing={1}
            >
              {/* ▼▼▼ CORREÇÃO 3 (a mais importante) AQUI ▼▼▼ */}
              {title && !titleBottom && <BTitle title={custom ? heading : item?.breadcrumb || item?.title} />}
