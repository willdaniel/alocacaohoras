...
class PgrService {
  constructor() {
    this.repository = new ConsultaRepository('colaboradores', ['id', 'nome', 'disciplina_id', 'data_pgr', 'status']);
  ...
  async getAll() {
    try {
      ...
      const sortedResults = sortByNearestDate(activeResults, 'data_pgr');

      return sortedResults.map((colaborador) => ({
        ...colaborador,
        disciplina: disciplinas.find((d) => d.id === colaborador.disciplina_id)?.nome || '',
        data_pgr: formatDate(colaborador.data_pgr, 'DD/MM/YYYY')
      }));
    } catch (error) {
